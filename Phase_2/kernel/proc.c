...
//        acquire(&p->lock);
//        winner = 0;
//        current_total_ticket = 0;
//        total_tickets = 0;
//        struct proc *p2;
//        for(p2 = proc; p2 < &proc[NPROC]; p2++)
//        {
//            if(p2->state==RUNNABLE){
//                total_tickets+=p2->tickets;
//            }
//        }
//        winner = getrand(total_tickets);
//        //TODO: fair random number??
//        for(p = proc; p < &proc[NPROC]; p++) {
//            //acquire(&p->lock);
//            //TODO: check acquire
//            if(p->state == RUNNABLE) {
//                current_total_ticket += p->tickets;
//            }
//            if(p->state == RUNNABLE && current_total_ticket >= winner ) {
...
//
//    p->num_processes = 0;
//    for (proc = proc; proc < &proc[NPROC]; proc++){//for each proc in table
...
int getprocessesinfo(struct processes_info *p) {
    ...
    int i=0;
    for (_proc = proc; _proc < &proc[NPROC]; _proc++){
        p->pids[i] = _proc->pid;
        p->ticks[i] = _proc->ticks;
        p->tickets[i] = _proc->tickets;
        i++;
    }
    return 0;
}
